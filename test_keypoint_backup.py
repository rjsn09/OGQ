...
while True:
    ...
    for result in results:
        ...
        for person_idx in range(num_people):
            ...
            if hands_up:
                if person_idx not in hands_up_timer or hands_up_timer[person_idx] is None:
                    hands_up_timer[person_idx] = current_time  #시작 시간 기록
                elapsed = current_time - hands_up_timer[person_idx]
                if left_wrist_y <= left_shoulder_y or left_wrist_y <= right_shoulder_y:
                    write_log(f"사람 {person_idx+1}, hands : {keypoints_xy[9][1]}, time : {elapsed}", True)
                if right_wrist_y <= left_shoulder_y or right_wrist_y <= right_shoulder_y:
                    write_log(f"사람 {person_idx+1}, hands : {keypoints_xy[10][1]}, time : {elapsed}", True)
                print(f"사람 {person_idx+1}, time : {elapsed}")

                if elapsed >= 5:  # 5초 이상 유지 시 박스
                    if boxes_obj is not None and boxes_obj.xyxy.shape[0] > person_idx:
                        ...
                        cv2.putText(annotated_frame, f"HandsUpPerson {person_idx+1}",
                                    (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.6, (0, 255, 0), 2)
            else:
                # 손 내려가면 초기화
                hands_up_timer[person_idx] = None

    cv2.imshow("YOLOv11 HandsUp", annotated_frame)
